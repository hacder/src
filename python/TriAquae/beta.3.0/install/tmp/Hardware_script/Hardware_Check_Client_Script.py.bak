...
import platform
import tri_config
triaquae_user=tri_config.Asset_collect_user
save_dir='/tmp'
old_passwd=tri_config.Asset_user_password
try:
    if sys.argv[1] == '-h':
        print '''
...
###check run user 
if os.getuid() == 0:
    print "\033[1;32;40m%s   %s\033[0m" %('This script run user is root!','Pass')
else:
    print "\033[1;31;40m%s   %s\033[0m" %('This script must be run as root!','No pass')
    sys.exit(1)
###check python version
now_version=((sys.version).split()[0])[0:3]
require_version='2.6'
if now_version >= require_version:
    print "\033[1;32;40m%s   %s\033[0m" %('Now python version is >2.6','Pass')
else:
    print "\033[1;31;40m%s   %s\033[0m" %('Now python version is <2.6','Not pass')
###check install modules
modules=['subprocess','platform','json','crypt','platform']
for i in modules:
    try:
        __import__(i)
    	print "\033[1;32;40m%s\033[0m" %'Now you system install require "%s" modules   %s'%(i,'Pass')
    except Exception,e:
...
if check_value == '1':
    print "\033[1;32;40m%s    %s\033[0m" %('Triaquae user in the /etc/passwd!','Pass')
else:
    print "\033[1;31;40m%s    %s\033[0m" %('Triaquae user not in the /etc/passwd','No pass')
    if option_auto == '-y':
...
if info in f.readlines():
    print "\033[1;32;40m%s   %s\033[0m" %('Triaquae user in the /etc/sudoers!','Pass')
else:
    print "\033[1;31;40m%s   %s\033[0m" %('Triaquae user not in the /etc/sudoers!','No pass')
    if option_auto == '-y':
