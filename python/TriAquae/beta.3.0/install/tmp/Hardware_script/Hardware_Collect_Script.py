...
Physical_Memory=((subprocess.Popen("/usr/sbin/dmidecode |grep -A16 'Memory Device$'|grep  'Size'|grep MB|awk '{print $2}'|awk '{a[$1]++}END{for(i in a){print i '*' a[i]}}'",shell=True,stdout=subprocess.PIPE)).stdout.readline()).strip('\n')
