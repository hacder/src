...
if(is_file($BASE_PATH."ctrlphp/etc/osa_config.inc.php")){
	...
	die( "osa_config.inc.sample.php文件丢失，请上传该文件到/usr/local/osa/ctrlphp/etc/目录");
}
...
if (isset($_GET['step'])){
    ...
    $step = 0;
	$param = array('step'=>'one');
	osa_restaction('POST',$param,OSA_WEBSERVER_DOMAIN.'/osa/install.php');
}	
...
//第三步，完成安装
	case 3:
	$param = array('step'=>'four','type'=>'安装向导');
	osa_restaction('POST',$param,OSA_WEBSERVER_DOMAIN.'/osa/install.php');	
	$targe_file=array('ctrlpy/etc/config.py');
