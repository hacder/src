$(document).ready(function(){
	...
	$("#add-options").click(function(){
		...
		if(numOptions>=maxOptions){
			tipsAlert("Mongodb自定义报警指标数量不能超过"+maxOptions);
		}else{
	...
	var getMongodbConfig = function(){	
		if($("#more-class").is(":visible")){
			var itemConfig = {};
			$(".col_4_con").each(function(){
				if($(this).find('.mongodb_norm').val() == "MongoDB当前连接数"){
					itemConfig.curr_connects={};
					itemConfig.curr_connects.condition = "大于";
					itemConfig.curr_connects.value = $.trim($(this).find(".threshold").val());
				}else if($(this).find('.mongodb_norm').val() == "MongoDB分页次数"){
					itemConfig.page_num={};
					itemConfig.page_num.condition = "大于";
					itemConfig.page_num.value = $.trim($(this).find(".threshold").val());
				}else if($(this).find('.mongodb_norm').val() == "MongoDB锁定时间比例"){
					itemConfig.lock_ratio={};
					itemConfig.lock_ratio.condition = "大于";
					itemConfig.lock_ratio.value = $.trim($(this).find(".threshold").val());
				}
			});
			return itemConfig;
		}else{
	...
	$("#mongodb-save").click(function(){
		...
		var itemconfig = getMongodbConfig();
		var url = "index.php?c=monitor&a=mongodb_monitor";
		$.post(url,{
			'itemname':itemname,
			'urlname':urlname,
	...
	$("#mongodb-edit").click(function(){
		...
		var url = "index.php?c=monitor&a=monitor_mongodb_edit";
		$.post(url,{
			'itemid':itemid,
			'itemname':itemname,
			'urlname':urlname,
