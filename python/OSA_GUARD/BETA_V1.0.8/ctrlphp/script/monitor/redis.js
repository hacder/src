$(document).ready(function(){
	...
	$(".tag_li").live('click',function(){
		var value = $(this).html();
		if(value == 'Redis使用内存'){
			$(this).parents(".col_4_con").find('.threshold-tips').html('KB');
		}else if(value == 'Redis命中率'){
	...
	$("#add-options").click(function(){
		...
		if(numOptions>=maxOptions){
			tipsAlert("Redis自定义报警指标数量不能超过"+maxOptions);
		}else{
	...
	var getRedisConfig = function(){	
		if($("#more-class").is(":visible")){
			...
			$(".col_4_con").each(function(){
				if($(this).find('.redis_norm').val() == "Redis链接客户数"){
					itemConfig.connected_clients={};
					itemConfig.connected_clients.condition = "大于";
					itemConfig.connected_clients.value = $.trim($(this).find(".threshold").val());
				}else if($(this).find('.redis_norm').val() == "Redis使用内存"){
					itemConfig.used_memory={};
					itemConfig.used_memory.condition = "大于";
					itemConfig.used_memory.value = $.trim($(this).find(".threshold").val());
				}else if($(this).find('.redis_norm').val() == "Redis命中率"){
					itemConfig.index_rate={};
					itemConfig.index_rate.condition = "大于";
					itemConfig.index_rate.value = $.trim($(this).find(".threshold").val());
				}
			});
			return itemConfig;
		}else{
	...
	$("#redis-save").click(function(){
		...
		var itemconfig = getRedisConfig();
		var url = "index.php?c=monitor&a=redis_monitor";
		$.post(url,{
			'itemname':itemname,
			'urlname':urlname,
	...
	$("#redis-edit").click(function(){
		...
		var url = "index.php?c=monitor&a=monitor_redis_edit";
		$.post(url,{
			'itemid':itemid,
			'itemname':itemname,
			'urlname':urlname,
