<div class="menu2">
	<p class="height10"></p>
	<p>
		<a class="menu2_title"><span>账户信息</span></a>
		<a href="index.php?c=account&a=userlists" class="menu2_title_sub curr_sub"><span>用户列表</span></a>

		<a href="index.php?c=account&a=rolelists" class="menu2_title_sub"><span>角色列表</span></a>

		<a href="index.php?c=account&a=passsave" class="menu2_title_sub"><span>修改密码</span></a>
		<a href="index.php?c=account&a=personset" class="menu2_title_sub"><span>个性化设置</span></a>
	</p>
	<p class="height10"></p>
	<p>
		<a class="menu2_title"><span>设备信息</span></a>
		<a href="index.php?c=device&a=listindex" class="menu2_title_sub curr_sub"><span>服务器列表</span></a>
		<a href="index.php?c=snmp&a=snmpset" class="menu2_title_sub"><span>全局snmp采集配置</span></a>
	</p>
	<p class="height10"></p>
	<p>
		<a class="menu2_title"><span>监控信息</span></a>
		<a href="index.php?c=monitor&a=monitorlist" class="menu2_title_sub curr_sub"><span>监控项目列表</span></a>
		<a href="index.php?c=monitor&a=itemlist" class="menu2_title_sub "><span>创建监控项目</span></a>
	</p>
	<p class="height10"></p>

</div>