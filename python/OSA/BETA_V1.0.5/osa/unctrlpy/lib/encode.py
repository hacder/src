...
from unctrlpy.etc.config import COMMANDS
...
def encode(cmdstr, _auth_key = COMMANDS['_AUTH_KEY'] , offset = COMMANDS['OFFSET']):
	...
	encode_str = ''.join( [ i.upper() if i.isalpha() and random.randint(0, 1)==1 else i for i in encode_str ] )
	encode_num = int((''.join([ i if i.isdigit() and int(i)>3 else "" for i in encode_str ]))[0:1])
	...
	enstr = COMMANDS['JAMSTR'] + '_____' + enstr + '_____' + str(len(canstr)) + '_____' + COMMANDS['JAMSTR']
	enlist = list(enstr)
	rstr = ''
