$(document).ready(function(){
	...
	function getPort(){
		var port = '';
		$(".check_port:checked").each(function(){
			port +=$(this).attr('value')+',';
		});
		var value = $.trim($("#input_port").val());
		if(value !=''&&!isPortstr(value)){
			return 'error';
		}
		port +=value;
		port = port.replace(/(^\,*)|(\,*$)/g, "");
	...
	function portSubmit(type){
		...
		var url = "index.php?c=panel&a=saveportalarm";
		if(type == 'edit'){
	...
	//提交
	$("#portconfirm").click(function(){
		var type="add";
		portSubmit(type);
	});
	$("#portalarmedit").click(function(){
		var type="edit";
		portSubmit(type);
	});
});