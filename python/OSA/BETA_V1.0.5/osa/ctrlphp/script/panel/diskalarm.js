$(document).ready(function(){
	...
	function getThreshold(){
		...
		if(value!=''&&!/^[0-9](\.\d+)?$|^[1-9][0-9](\.\d+)?$/.test(value)){
			return 'error';
		}
		return value;
	}
	
	function diskSubmit(type){
		flag = true ;
		$("#proname").blur();
		if(flag == false){
			alert('基本信息有错误');
		...
		var url = "index.php?c=panel&a=savediskalarm";
		if(type == 'edit'){
	...
	//提交
	$("#diskconfirm").click(function(){
		var type = 'add';
		diskSubmit(type);
	});
	$("#diskalarmedit").click(function(){
		var type = 'edit';
		diskSubmit(type);
	});
});